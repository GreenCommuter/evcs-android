#!/usr/bin/python
import os, glob, hashlib, pickle, argparse, shutil, ntpath, multiprocessing, signal, sys
...
import binascii
smallestpng = binascii.unhexlify('89504e470d0a1a0a0000000d49484452000000010000000108060000001f15c4890000000b4944415478da63f80f040009fb03fd68fa1ccc0000000049454e44ae426082')
...
def parseCommandLineOptions():
    ...
    dest="threads",
    help="number of threads to use to process the assets",
    metavar="N",
    ...
    buildGroup.add_argument("-d", "--dry-run",
    action="store_true",
    default=False,
    dest="dry_run",
    help="Creates blank images (useful to save bitrise building time)")
...
def classifyRawFiles(upToDateFiles, deletedFiles, newFiles, modifiedFiles):
    # Dictionary of previously hashed files: <file path, MD5 hash>
    storedHashedFiles = loadHashedFiles()
    # Dictionary of previously hashed files, but with only the filename as key
    storedHashedBasenameFiles = getDictionaryWithBasepath(storedHashedFiles)
    ...
    for path, md5 in recentlyHashedFiles.iteritems():
        basenamePath = ntpath.basename(path)
        if basenamePath in storedHashedBasenameFiles:
            if md5 == storedHashedBasenameFiles[basenamePath]:
            ...
            del storedHashedBasenameFiles[basenamePath] # Removed the processed entry
        else:
    ...
    for path, md5 in storedHashedFiles.iteritems():
        if md5 in storedHashedBasenameFiles.values():
...
# Return a dictionary with the md5 values and the basepath of the keys
def getDictionaryWithBasepath(oldDict):
    basepathDict = {}
    for path, md5 in oldDict.iteritems():
        basepathDict.update({ntpath.basename(path):md5})
    return basepathDict
...
def sendAssetToPngPipeline(rawAssetPath, density, processedAssetPath):
    if (shouldRunDry):
	with open(processedAssetPath, 'wb+') as f:
	    f.write(smallestpng)
            return
    filename = os.path.basename(rawAssetPath)
